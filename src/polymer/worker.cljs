...
(defn handle-message [message]
  (let [payload (js->clj (.-data message) :keywordize-keys true)
    ...
    (case (:type payload)
      ...
      "createGestureAgency" (create-system! id {:gesture (:config payload)})
      "createCameraContextAgency" (create-system! id {:cameraContext (:config payload)})
      "createTranscriptionAgency" (create-system! id {:transcription (:config payload)})
      "createConversationAgency" (create-system! id {:conversation (:config payload)})
      "createHairAgency" (create-system! id {:hair (:config payload)})
